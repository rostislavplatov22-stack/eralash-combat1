...
const PATCH_ID = '31.3 RENDERER RECOVERY';
const root = process.argv[2] ? path.resolve(process.argv[2]) : process.cwd();
const patchRoot = path.dirname(fileURLToPath(import.meta.url));

function fail(message) {
  console.error('[31.3 ERROR]', message);
  process.exit(1);
}
...
function pickIndex() {
  return walk(root)
    .map(file => {
      ...
      if (/hardCombat311|hard-combat-31-2-runtime|drawPremiumFighterArt|CHARACTER_SELECT_META/.test(text)) score += 12000;
...
const backupPath = path.join(projectDir, 'index.before-renderer-recovery-31.3.html');
