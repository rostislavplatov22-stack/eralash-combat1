...
import { listBosses, recordBossRun } from '../_missions.js';
...
export default async function handler(req, res) {
  try {
    if (req.method === 'POST') {
      ...
      res.status(200).json(await recordBossRun(userFrom(req, body), body));
      return;
    }
    res.status(200).json(await listBosses(userFrom(req, {})));
  } catch (error) {
